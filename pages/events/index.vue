	...
	export default {
		...
		methods: {
			search(){
				let that=this
				uni.request({
					url: that.$store.state.baseUrl+'/event/findByOther?keyword='+that.selectCondition.keyword+'&types='+that.selectCondition.types+'&time='+that.selectCondition.time+'&count='+that.selectCondition.count,
					header: {
			...
			init(){
				let that=this
				uni.request({
					url: that.$store.state.baseUrl+'/event/findByOther?keyword&types&time=all',
					header: {
						"Content-Type": "application/x-www-form-urlencoded"
					},
					method: 'GET',
			...
			selectEvent(val) {
				uni.navigateTo({
					url:'./eventDetail/index?type='+val.type+'&eventId='+val.eventId+'&userId='+val.userId
				})
			}
		}
