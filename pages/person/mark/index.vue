<template>
	<view class="wrap">
		...
		<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
			<view class="page-box">
				<view class="order" v-for="(res, index) in eventList" :key="res.event_event_id" @click="selectEvent({type:res.event_event_type,eventId:res.event_event_id,userId:res.event_event_userId})">
					<view class="top">
						<view class="left">
							<u-icon name="volume-up" :size="30" color="rgb(94,94,94)"></u-icon>
							<view class="store">{{ res.event_event_title }}</view>
						</view>
						<view class="right">{{ res.event_event_time | timeFilters}}</view>
					</view>
					<view class="item">
						<view class="content">
							<view class="title u-line-2">{{ res.event_event_info }}</view>
						</view>
					</view>
					<view class="total" v-show="res.event_event_phone!=''">
						联系电话:{{ res.event_event_phone}}
					</view>
					<view class="bottom">
						<view class="evaluate btn">{{res.event_event_type | typeFilters}}</view>
						<view class="right">
							<view class="btn btn2" @click.stop="delModal(res.mark_mark_id)">删除</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			del() {
				let that = this
				uni.request({
					url: that.$store.state.baseUrl+'/mark/' + that.markId,
					header: {
			...
			search() {
				let that = this
				uni.request({
					url: that.$store.state.baseUrl+'/mark/findByUserId?keyword=' + that.selectCondition.keyword + '&types=' + that.selectCondition.types +'&userId=' + that.$store.state.userInfo.user_id
			...
			init() {
				let that = this
				uni.request({
					url: that.$store.state.baseUrl+'/mark/findByUserId?keyword&types&time=all&userId=' + that.$store.state.userInfo.user_id,
					header: {
						"Content-Type": "application/x-www-form-urlencoded"
					},
					method: 'GET',
